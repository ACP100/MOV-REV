...
//               <div className="movie-rating">Rating: {review.rating}/5</div>
//               <p>{review.comment}</p>
//             </div>
//           ))
//         ) : (
    ...
    <div className="movie-details">
      ...
      <div className="reviews-section">
        ...
        {reviews.length > 0 ? (
          reviews.map((review) => (
            <div key={review.id} className="review-card">
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.5rem' }}>
                <h3 className="review-title">{review.title}</h3>
                <div className="movie-rating">⭐ {review.rating}/10</div>
              </div>
              <p style={{ color: '#cccccc', lineHeight: '1.6' }}>{review.comment}</p>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '0.5rem' }}>
                <p style={{ color: '#888', fontSize: '0.9rem' }}>
                  By: <strong style={{ color: '#ffa726' }}>{review.username}</strong>
                </p>
                {review.rating && (
                  <p style={{ color: '#888', fontSize: '0.9rem' }}>
          ...
          <p style={{ textAlign: 'center', color: '#cccccc', padding: '2rem' }}>
